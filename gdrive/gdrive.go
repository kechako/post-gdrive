...
func New(ctx context.Context) (*GDrive, error) {
	...
	config, err := google.ConfigFromJSON(b, drive.DriveScope, drive.DriveFileScope, drive.DriveAppdataScope)
	if err != nil {
...
func (g *GDrive) UploadFile(name string, parents []string, r io.Reader, pu googleapi.ProgressUpdater) (*drive.File, error) {
	file := &drive.File{
		Name:    name,
		Parents: parents,
	}

	c := g.service.Files.Create(file).Media(r)
	if pu != nil {
		c = c.ProgressUpdater(pu)
	}
...
func tokenCacheFile() (string, error) {
	...
	return filepath.Join(tokenCacheDir, url.QueryEscape("post-gdrive.json")), err
}
